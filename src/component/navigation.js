...
let Navigation = React.createClass({
    ...
    render() {
        ...
        return (
            <div className={classnames("ws-navigation", this.props.className)}>
                <ul className="ws-navigation-level1">
                    {_.map(data, value => (
                        <li key={value.key}>
                            <Flex alignCenter className={"ws-navigation-title" + this.getCurrentClassName(value.key)}>
                            ...
                            {value.sub && (
                                <Collapse in={value.open}>
                                    <ul className="ws-navigation-level2">
                                        {_.map(value.sub, val => (
                                            <li key={val.key}>
                                                <div
                                                    className={"ws-navigation-title" + this.getCurrentClassName(val.key)}
