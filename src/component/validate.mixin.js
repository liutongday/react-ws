...
var doValidate = function (options, rule, target) {
    ...
    if (result === true) {
        target.className = _.without(target.className.split(' '), className.error).join(' ');
    } else {
        target.className = _.union(target.className.split(' '), [className.error]).join(' ');
    }
};
...
var ValidateMixin = function () {
    ...
    return {
        ...
        validateTip: function (name) {
            ...
            return _.map(_.filter(options.tip, function (v) {
                return v !== true;
            }), function (v) {
                return v;
            });
